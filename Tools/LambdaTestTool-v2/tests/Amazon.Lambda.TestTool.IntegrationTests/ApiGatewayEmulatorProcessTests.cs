...
public class ApiGatewayEmulatorProcessTests(ITestOutputHelper testOutputHelper) : IAsyncLifetime
{
    ...
    private Task<int>? _emulatorTask;
    ...
    private async Task<HttpResponseMessage> TestEndpoint(string routeName, int apiGatewayPort, string httpMethod = "POST")
    {
        ...
        using (var client = new HttpClient())
        {
            ...
            while (DateTime.UtcNow - startTime < timeout)
            {
                ...
                {
                    lastException = ex;
                    testOutputHelper.WriteLine($"Request attempt failed: {ex.Message}");
    ...
    private async Task StartTestToolProcessAsync(ApiGatewayEmulatorMode apiGatewayMode, string routeName, int lambdaPort, int apiGatewayPort, CancellationTokenSource cancellationTokenSource, string httpMethod = "POST")
    {
        ...
        _emulatorTask = command.ExecuteAsync(context, settings, cancellationTokenSource);
    ...
    {
        testOutputHelper.WriteLine("Cleaning up the test...");
        if (_emulatorTask is null)
            Assert.Fail("The emulator task was null");
        await _emulatorTask;
    }
}
